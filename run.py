...
def get_db_connection():
    """
    Maakt een verbinding met de database en retourneert de verbinding.
    """
    try:
...
def register():
    if 'user_id' in session:
        return redirect(url_for('dashboard'))  # Als al ingelogd, ga naar dashboard
    ...
    if request.method == 'POST':
        try:
            ...
            # Zet gebruiker in de sessie en stuur door naar dashboard
            session['user_id'] = user_id
            session['username'] = username
            return redirect(url_for('dashboard'))

        except Exception as e:
            return render_template('register.html', error=f"Fout bij registratie: {e}")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user and check_password_hash(user[1], password):
                username = user[0]
                # Redirect naar dashboard met username als queryparameter
            ...
            return render_template('login.html', error=f"Er is een fout opgetreden: {e}")
